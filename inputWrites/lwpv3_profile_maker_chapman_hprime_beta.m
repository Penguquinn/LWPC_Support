function xx = lwpv3_profile_maker_chapman_hprime_beta(save_path, hprime, beta, h_0)

save_path = '\\wsl.localhost\Ubuntu-24.04\home\quinn\work\v3.0.1\';
    ...
    for bb=1:length(beta)
        for tt=1:length(h_0)
            if h_0(tt)>hprime(hh)
                ...
                if ~isfile([save_path basename '.prf'])
                    h = 60:2:100;

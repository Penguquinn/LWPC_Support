function [] = ripVLFp_function(Fs, Fs_nb, txname,Fc,filepath,outputpath)
% Fs = 100e3;  %broadband sampling frequency in Hz
% Fs_nb = 1;     %narrowband output sampling frequency in Hz
% txname = 'NAA'; %name of narrowband transmitter
% Fc = 24.0e3;    %transmitter center frequency in Hz
% filepath = 'H:\Hunter_Data\CN\2016\09\21\';         %filepath to broadband data
% outputpath = 'F:\VLF_Polarization\CN\2016\09\21\';   %filepath to narrowband output
...
if strcmp(name0(1:end-8),name1(1:end-8)) && strcmp(name0(end-4),'0') && strcmp(name1(end-4),'1')
    ...
    for offset = start_offset+30:30:bblength-60
        fprintf('%s %3.f Hz File %i/%i, %3.f%% complete\n',txname, Fs_nb, n-2,length(files)-2,offset/(bblength-60)*100)
...
function [s0data,s1data,s2data,s3data]=nb_polarization(data_ns,data_ew,Fc,Fs,Fs_nb)
    ...
    vlfdata = data_ns; %due to 180 degree ambiguity, this may be +/-
    ...
    vlfdata = data_ew; %due to 180 degree ambiguity, this may be +/-
    %apply filter forwards/backwards
    baseband = vlfdata.*mixdown.';
    baseband_filt = fftfilt(b,baseband);
    baseband_filt = fftfilt(b,baseband_filt(end:-1:1));
    ...
    s0sqdata = downsample(movmean(s0data.^2,2*Fs./Fs_nb),Fs./Fs_nb);
    s1sqdata = downsample(movmean(s1data.^2,2*Fs./Fs_nb),Fs./Fs_nb);
    s2sqdata = downsample(movmean(s2data.^2,2*Fs./Fs_nb),Fs./Fs_nb);
    s3sqdata = downsample(movmean(s3data.^2,2*Fs./Fs_nb),Fs./Fs_nb);
    
    s0sign = downsample(sign(movmean(s0data,2*Fs./Fs_nb)),Fs./Fs_nb);
    s1sign = downsample(sign(movmean(s1data,2*Fs./Fs_nb)),Fs./Fs_nb);
    s2sign = downsample(sign(movmean(s2data,2*Fs./Fs_nb)),Fs./Fs_nb);
    s3sign = downsample(sign(movmean(s3data,2*Fs./Fs_nb)),Fs./Fs_nb);

    
    s0data = sqrt(s0sqdata).*s0sign;
    s1data = sqrt(s1sqdata).*s1sign;
    s2data = sqrt(s2sqdata).*s2sign;
    s3data = sqrt(s3sqdata).*s3sign;
end

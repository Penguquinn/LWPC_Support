      SUBROUTINE    LWPM(lwpc_dloc,dc,file_name,nc)
       use, intrinsic :: iso_c_binding
c       implicit none
 
       character*120, intent(in) :: lwpc_dloc
c       character*120, intent(in) :: file_name
...
c  IONOSPHERE       Thomson MONTH/day/year hour:minute
...
c     Thomson MONTH/day/year hour:minute
c                   indicates that a specific date and time is to be
c                   used; the parameters of the ionospheric profiles
c                   are to be set by the program using the Thomson
c                   model; the parameters of the date and time is as
c                   follows:
c                      MONTH  name of the month
c                      day    day of the month
c                      year   year
...
c                     may be omitted by dropping "/year"; the day and
c                     year may be omitted by dropping "/day/year"; the
c                     year may be entered modulo (1900). The minutes may
c                     be omitted by dropping ":minute".


c  BEARINGS         Defines a list of angles used to define paths
...
c  PRINT-LWF        Levels of print out for the mode summation
...
c  PRINT-MDS        Levels of print out for the mode parameters
...
c   PRINT-SWG       Levels of print out for the calculations along the
c                   propagation paths
...
c  PRINT-MC         Levels of print from ModeConversion
...
c  PRINT-WF         Levels of print from WaveFields
...
c  and their associated data.  Control strings begin in column 1; if the
c  string is blank in column 1, then the string is treated as a comment;
c  a control string may be shortened so long as it is unique; more than
c  one space may separate the control string from its associated data
c  and any number of spaces may appear between data items.

c  NOTE:  When a list of control strings is provided, quantities that
c         appear in upper case are character data.
...
c  Change History:
c     23 Oct 1990   Deleted run-time definition of strings in argument
c                   lists of subroutines (using // operator).

c     05 Feb 1992   Initialize collision frequency to the "standard"
c                   values used by Wait's exponential profiles.

c     10 Feb 1992   Modified /LWPC_IN/ to store HOFWR,TOPHT,BOTHT and
c                   PINDEX.

c     22 May 1992   T.A.Hepner
...
c     01 Nov 1992   Added final configuration of the ModeConversion
c                   option.

c     20 Jan 1993   Added frequency dependent limits for RANGER and
c                   RANGEI.

c     10 Nov 1993   Added solar zenith angle dependent ionospheres.

c     11 Nov 1993   Disabled PREVIEW option for compatibility with
...
c     04 Jan 1994   In call to LWP_EOF_MDS, replaced argument EMPTY
c                   with NUMBER_OF_PATHS; removed status file.

c     06 Jan 1994   Added solar zenith angle to GCPATH output.

c     17 Jan 1994   Dropped FAT compatibility in GRD file names.

c     21 Jan 1994   Changed file naming convention for noise grids to
...
c     29 Jan 1994   Added ASCII/BINARY option for grid files.

c     06 Apr 1994   Added 3 standard deviations:
c                   (1: day; 2: night; and 3: transition).

c     30 Jun 1994   Dropped END_FILE_MDS from call to LWP_DRIVER;
c                   dropped PRFL_ID, CXCLT and SXCLT from call to
c                   OPA_NOISE_GRID.

c     21 Sep 1995   Fixed up handling of output:
...
c     26 Sep 1995   Changed all PRINT statements to WRITE(6) ones.

c     25 Oct 1995   Changed to use lwpc_LUN.cmn and lwpc_LUN.ini to set
c                   logical units.

c     18 Dec 1995   Added PRINT-MC and PRINT-WF to argument list.

c     30 Jan 1996   Added V sub d grid array.

c     12 Dec 1996   Modified to use new generic flush unit routine.

c     01 Mar 1997   Added option for presegmentation.

c     15 Jan 1998   Dropped PLT_UNIT; use PLT_DEVICE to store file name
c                   when output to a file.

c     05 Feb 1998   Removed control string FILE-DAT.

c     07 Sep 1998   Changed default values: FTOL=.1, MAXITR=5

c     03 Jul 1999   Added control string LWTIME to set date and time
c                   when non-LWPM ionosphere is used.

c     27 May 2003   Added date and time parameters to call to LW_VS_D.

c     23 Jun 2003   Modified to use Thomson model (PFLAG=11).
...
c     Parameters for LWPM
      parameter    (mxpath=120,mxprm1=21,
     &              mxeigen=50,mxprm2=5,mxsgmnt=2001,
...
c     &              CMDLine, lwpcDAT_loc
     ...
     &              dist_avg
      ...
      integer       bflag,pflag,pindex,year,day,UT,
...
c     SW Common
     ...
     &              ftol/.1/,mxiter/1001/,
     &              prec/2./,rpoly/1/,nrtlst/5/,
c     WF Common
     &              dtheta2/.01,.001/,tol2/.05,.005/,thtinc2/.5/,
     &              h2/50./,prec2/2./,
     &              iter_flag/1/,mxiter2/1001/,
c     Miscellaneous
...
c     Programmer selectable
     ...
     &              month/7/,day/15/,year/1984/,UT/1615/,
...
c     set consistent values for alpha in real and double precision

      alpha = 2.0 / REAL(r_earth)
      alpha2 = 2.d0 / r_earth

c     Get the location of the LWPC data
c      call LWPC_DAT_LOC
      lwpcDAT_loc = lwpc_dloc(1:dc)

c     Get the command line
c      call GET_COMMAND_LINE(lenCMD,CMDLine)

c     For this program we assume that there is only the input file name.
c      if (lenCMD .eq. 0) then
c         file_name=pgm_name
c      else
c         file_name=CMDLine
c      end if
      file_name(nc+1:) = ' '
...
c      call GETTIM (c_hrs,c_mins,c_secs,c_hsecs)
...
c        Print date/time of completion of run.
         call GET_DATE (c_year,c_month,c_day)
c         call GET_TIME (c_hrs,c_mins,c_secs,c_hsecs)
         call date_and_time(date,time)
         write(systime,
     & '(a4,''-'',a2,''-'',a2,'' '',a2,'':'',a2,'':'',a2)') 
     & date(1:4),date(5:6),date(7:8), 
...
c         write(lwpcLOG_lun,
c     &       '(/''Run finished on '',i2.2,''-'',i2.2,''-'',i4.4,
c     &          '' at '',2(i2.2,'':''),i2.2)')
c     &          c_month,c_day,c_year,c_hrs,c_mins,c_secs
         STOP
      else
...
c        Loop over path bearings
     ...
     &               case_id(:max0(1,ISTR_LENGTH(case_id))),
...
c        File for storage of mode data
         if (ISTR_LENGTH(lwpcMDS_loc) .gt. 1) then
            file_name=lwpcMDS_loc(:ISTR_LENGTH(lwpcMDS_loc))//
     &                root_file(:nfchr)//'.mds'
         else
...
c        Get header data from the mode parameter file
     ...
     &        mds_format)
         ...
         if (ISTR_LENGTH(lwpcLWF_loc) .gt. 1) then
            file_name=lwpcLWF_loc(:ISTR_LENGTH(lwpcLWF_loc))//
     &                root_file(:nfchr)//'.lwf'
         else
...
c           Attach the directory name
            if (ISTR_LENGTH(lwpcGRD_loc) .gt. 1) then
               file_name=lwpcGRD_loc(:ISTR_LENGTH(lwpcGRD_loc))//
     &                   root_file(:nrchr_f)//
     &                   area_id(:nrchr_a)//'.grd'
            else
...
c           Now that the frequency is known, generate noise data grid.
            if (pflag .eq. 4 .or. pflag .eq. 11 .or.
     &          ISTR_LENGTH(n_model) .gt. 0) then

               if (pflag .eq. 4 .or. pflag .eq. 11) then

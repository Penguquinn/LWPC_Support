...
c  Change History:
c     06 Apr 1994   Smooth the mode sum over 5 points;
c                   added 4 standard deviations (1: day; 2: night;
c                   3: dawn transition, 4: dusk transition).

c     13 Jan 1995   Corrected EOF error when more than one component is
c                   in the LWF file.
...
c*******************!***************************************************
      ...
      parameter    (mxprmx=21,mxps=11,mxsgmnt=2001)
...
c     Extract profile identification for specified date and time
      if (prfl_id(:10) .eq. 'LWPM Date:' .or.
     &    prfl_id(: 2) .eq. 'ZZ') then

c        Variable diurnal conditions along the paths
         diurnal=.true.

c        Extract the date and time
         if (prfl_id(:10) .eq. 'LWPM Date:') then
            read (prfl_id,'(11x,3(i2,1x),i4)')
     &            month,day,year,UT
         else
            read (prfl_id,'(3x,3(i2,1x),i4)')
     &            month,day,year,UT
         end if
...
c        call ALMNAC (year,month,day,UT_hours,ssclt,sslng)
         ...
         call GEO_SUBSOLAR (year,month,day,UT_hours,slat,slon)
...
c        Fixed diurnal condition along the paths
         ...
         if (prfl_id(:4) .eq. 'LWPM') then

c           LWPM model
            if (prfl_id(6:8) .eq. 'DAY') then

      SUBROUTINE GCDBR2
     &          (dphi,clt1,cclt1,sclt1,clt2,cclt2,sclt2,rho,br,inb)

c***********************************************************************

c  Function:
c     Returns great circle distance and geographic bearing angle from
c     position 1 (P1) to position 2 (P2).

c  Parameters passed:
c     dphi          [r] longitude of P1 minus longitude of P2
c     clt1          [r] co-latitude of P1
c     cclt1         [r] cosine of clt1
c     sclt1         [r] sine   of clt1
c     clt2          [r] co-latitude of P2
c     cclt2         [r] cosine of clt2
c     sclt2         [r] sine   of clt2
c     rho           [r] great circle distance from P1 to P2
c     inb           [i] =0: RHO is computed and
c                           BR from P1 thru P2 is computed at 1
c                       =1: RHO is input    and
c                           BR from P1 thru P2 is computed at 2

c  Parameters returned:
c     rho           [r] great circle distance from P1 to P2
c     br            [r] geographic bearing angle of the path

c  Notes:
c     Sign convention is + for West and North.
c     All coordinates are in radians.

c  Common blocks referenced:

c  Functions and subroutines referenced:
c     abs
c     acos
c     cos
c     min
c     mod
c     sign
c     sin

c  References:

c  Change History:

c*******************!***************************************************

      include 'constants.inc'

      REDUCE(arg)=SIGN(MIN(ABS(arg),1.),arg)


      dl=MOD(dphi,REAL(twopi))
      adl=ABS(dl)
      if (ABS(clt1) .eq. 0. .or. ABS(clt1-pi) .eq. 0.) then
c        Point 1 is at one of the poles
         gcd=ABS(clt1-clt2)
         if (ABS(clt1) .eq. 0.) then
            br=REAL(pi)-dl
         else
            br=dl
         end if
         if (br .lt. 0.) br=br+REAL(twopi)
      else
     &if (adl .eq. 0.) then
c        Coordinates are on same longitude
         gcd=clt1-clt2
         if (gcd .ge. 0.) then
            br=0.
         else
            gcd=-gcd
            br=REAL(pi)
         end if
      else
     &if (ABS(adl-pi) .eq. 0.) then
c        Coordinates are on opposite longitudes
         gcd=clt1+clt2
         if (gcd .le. pi) then
            if (inb .eq. 0) then
               br=0.
            else
               br=REAL(pi)
            end if
         else
            gcd=REAL(twopi)-gcd
            if (inb .eq. 0) then
               br=REAL(pi)
            else
               br=0.
            end if
         end if
      else
         if (adl .ge. pi) then
            if (dl .ge. 0.) then
               dl=dl-REAL(twopi)
            else
               dl=dl+REAL(twopi)
            end if
         end if
         if (inb .eq. 0) then
            cgcd=cclt1*cclt2+sclt1*sclt2*COS(dl)
            gcd=ACOS(REDUCE(cgcd))
            if (gcd .eq. 0.) then
               br=0.
            else
               if (sclt1 .eq. 0.) then
c                 Point 1 is at one of the poles
                  if (ABS(clt1) .lt. piov2) then
                     br=REAL(pi)
                  else
                     br=0.
                  end if
               else
                  br=ACOS(REDUCE((cclt2-cclt1*cgcd)/
     &                                 (sclt1*SIN(gcd))))
               end if
            end if
         else
            if (rho .gt. pi) then
               gcd=REAL(twopi)-rho
            else
               gcd=rho
            end if
            if (gcd .eq. 0.) then
               br=0.
            else
               if (sclt2 .eq. 0.) then
c                 Point 2 is at one of the poles
                  if (ABS(clt2) .lt. piov2) then
                     br=REAL(pi)
                  else
                     br=0.
                  end if
               else
                  br=REAL(pi)-ACOS(REDUCE((cclt1-cclt2*COS(gcd))/
     &                                    (sclt2*SIN(gcd))))
               end if
            end if
         end if
         if (dl .lt. 0.) br=REAL(twopi)-br
      end if
      if (inb .eq. 0) then
         rho=gcd
      else
         if (rho .gt. pi) then
c           Long path calculations
            if (br .lt. pi) then
               br=br+REAL(pi)
            else
               br=br-REAL(pi)
            end if
         end if
      end if

      RETURN
      END      ! GCDBR2

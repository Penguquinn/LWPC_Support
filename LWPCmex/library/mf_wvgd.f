...
c  Change History:
c     26 Oct 1995   Changed to get the LOG unit from LWPC_LUN.CMN.

c     26 Apr 1996   Initialize direction cosines.
...
c     LWPC parameters
     ...
     &              nrboxs,kb
     &      /mf_crnr/
     &              tcornr(mxcorners),rcornr(4,mxcorners),nrcorner
     &      /mf_drcs/
     &              dcl,dcm,dcn,ec,g,q
     &      /mf_eigb/
     &              eigenb(mxeigenb),nreigenb
     &      /mf_flag/
...
c     Initialization
      dcl= COS(dip*dtr)*COS(azim*dtr)
      dcm= COS(dip*dtr)*SIN(azim*dtr)
      dcn=-SIN(dip*dtr)
...
20    trht=boxrht(kb)
     ...
     &   write(lwpcLOG_lun,
     &       '(''MF_WVGD: Box'',i3,3x,''theta real='',f7.2,1x,
     &         ''to'',f7.2,3x,''theta imag='',f7.2,'' to'',f7.2)')
     &           kb,tlft,trht,ttop,tbot
      ...
      if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &   write(lwpcLOG_lun,
     &       '(''MF_WVGD: NOTLIN set - box divided'')')
      go to 20

21    if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &   write(lwpcLOG_lun,
     &       '(''MF_WVGD: NOTLIN set - MF_LAGRNG instead'')')
22    call MF_INITLG
      call MF_INITLG
      if (nointg .ne. 0) go to 51
...
c        Set up heights for FSINTG
      ...
      if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &   write(lwpcLOG_lun,
     &       '(''MF_WVGD: NOFINL set - box divided'')')
      go to 20
...
Change 22 Feb 1994: Dropped printing list of eigens
         if (lprnt .gt. 0 .or. mrprnt .gt. 0) then
            if (noevct .ne. 0)
     &         write(lwpcLOG_lun,
     &             '(''MF_WVGD: ROE not used'')')
            write(lwpcLOG_lun,
     &          '(''MF_WVGD: '',2i3,2f8.3,10x,2f8.3)')
     &              nreigen+1,jb,eigenb(jb),udfdt
         end if
...
52    if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &   write(lwpcLOG_lun,
     &       '(''         No modes found in this box'')')

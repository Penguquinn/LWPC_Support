...
c     20 Nov 1996   Added test for SZSQ2D to be zero to prevent division
c                   by zero.
...
c     LWPC parameters
     ...
     &      /mf_rntg/
     &              r(4),dr(4),drdh(4),drdcdh(4),
     &              roe(4),droe(4),rnd(5)
     &      /mf_roes/
     &              rlist(22,mxboxes)
     &      /mf_rseq/
     &              r0(4),drdt(4),d2rdt2(4),d3rdt3(4),
...
c        the magneto-ionic reflection coefficients.
     ...
     &                  ''Cycles of log ROE cannot be set'')')
            ...
            rgiven(k,1)=rgiven(k,1)+trin(1,mmin)*(0.,1.)*REAL(twopi)
            rgiven(k,2)=rgiven(k,2)+trin(2,mmin)*(0.,1.)*REAL(twopi)
            go to 41
         end if
...
c           Add or subtract whole cycles of phase.
            rgiven(k,1)=rgiven(k,1)+trin(2,mmin)*(0.,1.)*REAL(twopi)
            rgiven(k,4)=rgiven(k,4)+trin(2,mmin)*(0.,1.)*REAL(twopi)
            go to 52
         end if

...
c     Corners of op area
      ...
      xlng1=xlon1*dtr
      xlng2=xlon2*dtr
      if (xlng2 .ge. xlng1) xlng2=xlng2-REAL(twopi)
...
c           Transmitter is inside op area
            ...
            rbear(2)=360.-rbear(3)
            rhom(1)=0.
            rhom(2)=MIN(20000.,1000.*AINT((tclt+xclt1)*rtx+.9))
         else
...
c              Antipode is in operating area
               rbear(1)=0.
               rbear(2)=360.-rbear(3)
               rhom(1)=MIN(20000.,1000.*AINT((tclt-xclt1)*rtx))
               rhom(2)=20000.
            else
...
c           Transmitter is inside op area
            ...
            rhom(1)=0.
            rhom(2)=MIN(20000.,
     &                  40000.-1000.*AINT((xclt2+tclt)*rtx+.9))
         else
...
c              Antipode is in operating area
               rbear(1)=0.
               rbear(2)=360.-rbear(3)
               rhom(1)=MIN(20000.,1000.*AINT((xclt2-tclt)*rtx))
               rhom(2)=20000.
            else
...
c           Get ranges to corners of the op area
            dl=tlng-xlng1
            if (ABS(dl) .gt. pi) dl=dl-SIGN(REAL(twopi),dl)
     ...
     &                   r22,b22,0)
            rhom(1)=0.
            rhom(2)=MIN(20000.,
     &                   1000.*AINT(MAX(r11,r12,r21,r22)*rtx+.9))
            RETURN
         end if
      end if
...
c           Get ranges to corners of the op area
     ...
     &                   r22,b22,0)
            rhom(1)=MIN(20000.,
     &                   1000.*AINT(MIN(r11,r12,r21,r22)*rtx))
            rhom(2)=20000.
            RETURN
         end if
      end if
...
c        Get tangent bearings
         ...
         bmin=REAL(twopi)-angl
         bmax=REAL(twopi)+angl
...
c        Compute distances to corners of the op area
     ...
     &                r22,b22,0)
         ...
         if (bmax .gt. b21+twopi) bmax=b21+REAL(twopi)
     ...
     &       rbear(2)=rbear(2)-rbear(3)
         ...
         rhom(1)=MIN(20000.,1000.*AINT(rmin*rtx))
         rhom(2)=MIN(20000.,1000.*AINT(rmax*rtx+.9))
         RETURN
      else
...
c     Compute bearings to corners of the op area
      ...
      call GCDBR2 (dl,tclt,ctclt,stclt,xclt1,cxclt1,sxclt1,
     &             r11,b11,0)
      if (flagb .and. dl .ge. 0.) b11=b11+REAL(twopi)
      call GCDBR2 (dl,tclt,ctclt,stclt,xclt2,cxclt2,sxclt2,
     &             r12,b12,0)
      if (flagb .and. dl .ge. 0.) b12=b12+REAL(twopi)
      dl=tlng-xlng2
      if (ABS(dl) .gt. pi) dl=dl-SIGN(REAL(twopi),dl)
      call GCDBR2 (dl,tclt,ctclt,stclt,xclt2,cxclt2,sxclt2,
     &             r21,b21,0)
      if (flagb .and. dl .ge. 0.) b21=b21+REAL(twopi)
      call GCDBR2 (dl,tclt,ctclt,stclt,xclt1,cxclt1,sxclt1,
     &             r22,b22,0)
      if (flagb .and. dl .ge. 0.) b22=b22+REAL(twopi)
...
c           Antipode is above operating area
            rmin=MIN(rmin,REAL(twopi)-tclt-xclt1)
            rmax=MAX(rmax,REAL(twopi)-tclt-xclt2)
         else

...
c     Local
      ...
      integer       print_hgts,pindex,prev_ndx
      real          item(2)

      dimension     zna(mxchi),ndx(mxchi)
...
c              Check for comment line
     ...
     &             ISTR_LENGTH(string) .gt. 0) then

c                 Segment data should contain: chi pindex
                  call DECODE_LIST_FLT (string,2,nritem,item)
                  if (nritem .ne. 2) then
                     write(error_msg,
     &                   '(''[PRFL_CHI_TBL]: '',
     &                     ''Chi and index not all input'')')
                     call LWPC_ERROR('ERROR', error_msg)
                  end if

                  nrchi=nrchi+1
                  zna(nrchi)=item(1)
                  ndx(nrchi)=INT(item(2))
               end if
            end do
...
c        simply initializes the profile parameters.
         if (ISTR_LENGTH(directory) .eq. 0) then
            if (ISTR_LENGTH(lwpcPRF_loc) .eq. 0) then
               file_name=root_file(:ISTR_LENGTH(root_file))//'000.prf'
            else
               file_name=lwpcPRF_loc(:ISTR_LENGTH(lwpcPRF_loc))//
     &                   root_file(:ISTR_LENGTH(root_file))//'000.prf'
            end if
         else
            file_name=directory(:ISTR_LENGTH(directory))//
     &                root_file(:ISTR_LENGTH(root_file))//'000.prf'
...
c        from PRFL_INIT_TABLE.
         prfl_id='Chi Tbl: '//string(:ISTR_LENGTH(string))
...
c           Check for a file named "PRFL_FILE"nnn.PRF
     ...
     &                  root_file(:ISTR_LENGTH(root_file)),pindex
               else
                  write(file_name,'(a,a,i3.3,''.prf'')')
     &                  lwpcPRF_loc(:ISTR_LENGTH(lwpcPRF_loc)),
     &                  root_file(:ISTR_LENGTH(root_file)),pindex
               end if
            else
               write(file_name,'(a,a,i3.3,''.prf'')')
     &               directory(:ISTR_LENGTH(directory)),
     &               root_file(:ISTR_LENGTH(root_file)),pindex
            end if
...
c                 Unformatted profile
     ...
     &                    prfl_model(:MAX(1,ISTR_LENGTH(prfl_model)))
                  call LWPC_ERROR('ERROR', error_msg)
               end if
            else

...
Change: 20 Feb 1992 This parameter added
c     11)pindex     profile index for non-exponential profiles

Change: 06 Mar 1992 This parameter modified by LW_FULL_MC
...
Change: 14 Mar 1991 These parameters may be added by LW_VS_D
...
c  Change History:
c     14 Mar 1991   The parameter list may contain the date and time if
c                   LWPM was run for a specific date and time.

c     20 Feb 1992   Added profile index to PARAM.

c     06 Mar 1992   Profile index modified by LW_FULL_MC.

c     08 Feb 1994   Added error checking on write statements.

c     21 Oct 1995   Changed to get the LOG unit from LWPC_LUN.CMN.

c     27 May 2003   Added printout of date and time if the information
c                   is in the parameter list.
...
c              The fields were generated for a specific date and time
c              (probably using a non-LWPM ionosphere), so extract the
c              parameters for the label.
               month=INT(param( 7))
               day  =INT(param( 8))
               year =INT(param( 9))
               UT   =INT(param(10))
     ...
     &              (param(i),i=1,6),month,day,MOD(year,100),UT
            else
               write(lwpcLOG_lun,
     &             '(i2,i5,f7.1,f7.0,f6.1,f7.1,f7.0,
     &               f7.0,f8.0,f5.0,f7.1,2f5.1)')
...
c        The file is open
     ...
     *       (open_format(1:1) .eq. 'u' .and.
     ...
     &          '(''[WRITE_LWF]: ''
     &            ''Writing file: '',a)')
     &              open_name(:ISTR_LENGTH(open_name))
         else

...
c     capk=1./(1.-.5*alpha*h
      capk=SQRT(1.-alpha*h)
Change: END
      omega=1.d3*twopi*freq
      wn= (1.d6*twopi/c_speed)*freq
...
c           Calculate magnitude of the F function and polarization
            ...
            if (ABS(term1) .gt. ABS(term2)) then
               pmag=REAL(ABS((zone-rbar22*r22)/term1))
            else
               pmag=REAL(ABS(rbar11*r12/term2))
            end if
...
c                 stp=s*capk
                  stp=s/capk
Change: END
                  at=REAL(-8.6858896d3*wn*DIMAG(stp))
                  vc=REAL(1.d0/DREAL(stp))
                  tp(kn)=CMPLX(-zmplxi
     &                  *LOG(SQRT(zone-stp*stp)+zmplxi*stp)/zdtr)
                  ...
                  wm=REAL(REAL(cxlog*8.685889638d0))
                  wa=REAL(AIMAG(cxlog))
                  if (wa .lt. 0.d0) wa=wa+REAL(twopi)
                  if (print_swg .gt. 1)
     &               write(lwpcLOG_lun,
     ...
     &                     at,vc,wm,wa
                  ...
                  capt(1,kn)=CMPLX(store1/norm11)
                  capt(2,kn)=CMPLX(store3/norm22)
                  capt(3,kn)=CMPLX(store2/norm12*r21)
                  capt(4,kn)=CMPLX(r12/r21)
...
c                    Check atenuation rate
                     atten=
     &                  REAL(-8.686d3*wn/capk*DIMAG(SIN(theta0*zdtr)))

...
c        using the coarse bearing resolution
     ...
     &       brngmx .eq. 360.) brngmx=brngmx-brsltn(1)
         nrpath=0
c fixed real do variable for f95 compliance 
c        do br=brngmn,brngmx,brsltn(1) 
         do ib=1,INT((brngmx-brngmn)/brsltn(1))
            br=brngmn+(ib-1)*brsltn(1)
            nrpath=nrpath+1
            brng(nrpath)=br
            call LWP_TO_EDGE

...
c     LWPC parameters
     ...
     &              argln,qmag,zsqmgp,zsqmgm,zsqmag,rtd,dtr
     ...
     &   write(lwpcLOG_lun,
     &       '(''MF_BRNCPT: Closed solution for branch points'')')
...
c           is included.
            ...
            bpt(i)=(90.,0.)-tcompl*rtd

            if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &         write(lwpcLOG_lun,
     &             '(''MF_BRNCPT: bpt='',2f10.3,'' q='',1p2e10.3,1x,
     &               ''zsqmag='',e10.3)')
     &                 bpt(i),qq(i),zsqmag
            i=i+1
            qp=(dcn*t(ii)-rtterm)/(dclsq+dcnsq)
...
c     are not to be used.
      ...
      if (ec .le. 0.) then
         qpt=CMPLX(ACOS(SQRT(-ec))*rtd,0.)
      else
         argln=SQRT(ec)+SQRT(1.+ec)
         qpt=(90.,0.)+(0.,-1.)*LOG(argln)*rtd
      end if
      bpt(6)=qpt
      qtest=SQRT(COS(qpt*dtr)**2+ec)
      qmag=ABS(qtest)

      if (lprnt .ne. 0 .or. mrprnt .ne. 0)
     &   write(lwpcLOG_lun,
     &       '(''MF_BRNCPT: qmag='',1pe9.2,'' at theta='',0p2f7.3)')
     &           qmag,qpt
      RETURN
      END      ! MF_BRNCPT

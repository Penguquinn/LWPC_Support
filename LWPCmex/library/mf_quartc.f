...
c*******************!***************************************************
     ...
     &              omega2/(-0.5,-0.8660254038)/
      ...
      b3 sq=b3**2
      h=CMPLX(b2-b3 sq)
      i=CMPLX(b0-4.*b3*b1+3.*b2**2)
      g=CMPLX(b1+b3*(-3.*b2+2.*b3 sq))
      h prime=-i/12.
      ...
      if (ABS(g) .ge. 1.e-20) then
         sign=REAL(-root p*root q*root r*2./g)
         if (sign .lt. 0.) root r=-root r
      end if
      q(1)=CMPLX(+root p+root q+root r-b3)
      q(2)=CMPLX(+root p-root q-root r-b3)
      q(3)=CMPLX(-root p+root q-root r-b3)
      q(4)=CMPLX(-root p-root q+root r-b3)

...
c     Conductivity map data
      ...
      character*200 file_name,error_msg
      logical       first

      dimension     ss(10),rr(10)

      data          file_name/'cond_d.dat'/,
     &              first/.true./,
...
c        Put the conductivity values in common /GRND_DATA/
         do l=1,10
            sss(l)=ss(l)
            rrr(l)=rr(l)
         end do
...
c     Find the latitude strip
      lat=INT(181.-2.*xlat)
      if (lat .lt.   1) lat=  1
      if (lat .gt. 360) lat=360

      lon=INT(361.-2.*phi)
      if (lon .lt. 1 .or. lon .gt. 720) lon=1

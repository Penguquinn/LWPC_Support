...
c  Change History:
c     05 Mar 1997   Modified for tabular specification of beta and
...
c        NRSLAB is the total number of segments defining night through
c        day, including night, day and the transition, and vice versa
         nrslab=nrterm+2

c        NRPRFL is the total number of segments defining the complete
c        diurnal variation
         nrprfl=nrslab*2-1
...
c                    Noon to midnight
     ...
     &            month,day,MOD(year,100),UT
         end if
         if (ISTR_LENGTH(extra_id) .gt. 0)
     &      prfl_id(ISTR_LENGTH(prfl_id)+2:)=extra_id

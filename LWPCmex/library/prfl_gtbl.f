...
c        from PRFL_INIT_TABLE.
         prfl_id='Grd Tbl: '//string(:ISTR_LENGTH(string))
...
c           Direct access file for PRFLSCN
            if (ISTR_LENGTH(directory) .eq. 0) then
               if (ISTR_LENGTH(lwpcPRF_loc) .eq. 0) then
                  file_name=root_file(:ISTR_LENGTH(root_file))//'.prf'
               else
                  file_name=lwpcPRF_loc(:ISTR_LENGTH(lwpcPRF_loc))//
     &                      root_file(:ISTR_LENGTH(root_file))//'.prf'
               end if
            else
               file_name=directory(:ISTR_LENGTH(directory))//
     &                   root_file(:ISTR_LENGTH(root_file))//'.prf'
...
c        Find the profile index
         ...
         nlon=INT((xlon_min-tmp)/xlon_inc)+1
         nlat=INT((lat-xlat_min)/xlat_inc)+1

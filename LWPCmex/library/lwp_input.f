...
c     PLOT-DEVICE   Plotter device (SYS-SCN/SYS-PRN/FILE-NAME)
...
c  Change History:
c     23 Oct 1990   Modified to use the more rigorous testing of maps,
c                   op areas and transmitters found in GET_MAP, GET_OPA
c                   and GET_XMTR.

c     28 Feb 1991   Modified to accept MF printout flags through DATUM.

c     14 Mar 1991   Store noise year MOD 100.

c     30 Jan 1992   Modified to use DECODE_* routines.

c     10 Feb 1992   Modified /LWPC_IN/ to store HOFWR, TOPHT, BOTHT and
c                   PINDEX

c     27 Feb 1992   Deleted /LWPC_PR/.
...
c     11 Nov 1993   Removed PREVIEW control string.

c     30 Dec 1993   Added control strings for LWTime and SetupLWT.

c     17 Jan 1994   Removed unused common MF_SW_2.

c     09 Feb 1994   Changed PLT_DEVICE and PLT_ORIENTATION to
c                   CHARACTER*(*).

c     22 Feb 1994   Added FILE-LWT to specify the location of the LWT
c                   files created and used by LWTime.

c     20 Apr 1995   Ensure that the 'lwpcXXX_loc' end with '\'.

c     26 Sep 1995   Changed all PRINTs to WRITE(6)s.

c     21 Oct 1995   Changed to get logical units from LWPC_LUN.CMN.

c     18 Dec 1995   Added PRINT-MC and PRINT-WF parameters to
c                   argument list.

c     16 Dec 1996   Generalize delimiters used with file names.

c     04 Mar 1997   Added options for presegmentation.
...
c     LWPC parameters
      ...
      parameter    (mxpath=120,mxsgmnt=2001,mxeigen=50)
...
c           Current directory is default
            ...
            lwpcLWF_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcLWF_loc)
...
c           Current directory is default
            ...
            lwpcGRD_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcGRD_loc)
...
c           Current directory is default
            ...
            lwpcMDS_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcMDS_loc)
...
c           Current directory is default
            ...
            lwpcLWT_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcLWT_loc)
...
c           Current directory is default
            ...
            lwpcPRF_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcPRF_loc)
...
c           Current directory is default
            ...
            lwpcNDX_loc=data_string(1:ISTR_LENGTH(data_string))
            n=ISTR_LENGTH(lwpcNDX_loc)
...
cxx     &          map_id(:ISTR_LENGTH(map_id)),map_prjctn,
...
cxx     &          area_id(:ISTR_LENGTH(area_id)),
...
c        Read precomputed segmentation data
     ...
     &          (data_string,10,nrlist,preseg_list)
            ...
            if (rho .lt. 40000.) then
               ...
               ncd(nrsgmnt)=INT(preseg_list( 5))
               sgm(nrsgmnt)=preseg_list( 6)
               eps(nrsgmnt)=preseg_list( 7)
               npr(nrsgmnt)=INT(preseg_list( 8))
...
cxx     &          xmtr_id(:ISTR_LENGTH(xmtr_id)),

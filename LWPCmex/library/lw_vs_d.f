...
c     28 Dec 1995   Test running average of the mode sum; terminate
c                   if 5 consecutive points drop below -30 dB; store
c                   the running average instead of the computed fields.

c     27 May 2003   Added date and time parameters to argument list
...
c     LWPC parameters
      ...
      parameter    (mxprm=21,mxps=11,mxsgmnt=2001)
      ...
      integer       pflag,pindex,day,year,UT
...
c      write(*,*) "LW_VSD CALLED"
c     Set up distance array
      nrdst=INT((dmax-dmin)/dinc)+1
...
c              Set parameter list for the output files
               param( 1)=power
               param( 2)=0.
               param( 3)=incl
               param( 4)=headng
...
c                 The lw fields are to be generated for a specific date
c                 and time, so include these parameters in the list.
                  param( 7)=month
                  param( 8)=day
                  param( 9)=year
                  param(10)=UT

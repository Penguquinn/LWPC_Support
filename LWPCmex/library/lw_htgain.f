...
c*******************!***************************************************
     ...
     &              p1,h1z,h2z,h1pz,h2pz
      ...
      wn= 1.e6*REAL(twopi/c_speed)*freq
      ak=alpha/wn
      ak13=EXP(LOG(ak)/3.)
      ak23=ak13**2
      ka13=1./ak13
      ka23=ka13**2
      ngsq=CMPLX(epsr,-sigma/(1e3*REAL(twopi*eps_0)*freq))
      ...
      do neigen=1,nreigen
         thetap=tp(neigen)
         ssq=CSIN(tp(neigen)*REAL(pi)/180.)**2
         ...
         if(AIMAG(tp(neigen)) .le. -10. .or. alpha .eq. 0.) then
            ...
            hg(1,neigen)= expz+CMPLX(a1)/expz
            hg(2,neigen)= expz+CMPLX(a2)/expz
            hg(3,neigen)=(expz-CMPLX(a1)/expz)*c
            if(iopt .eq. 2) then
               hg(1,neigen)=hg(1,neigen)/( (1.,0.)+CMPLX(a1))
               hg(2,neigen)=hg(2,neigen)/( (1.,0.)+CMPLX(a2))
               hg(3,neigen)=hg(3,neigen)/(((1.,0.)-CMPLX(a1))*c)
            end if
         else
...
c           hg2=-i*ka13*sqroot*a1+a2
            ...
            hg(1,neigen)=CMPLX(hg1*DEXP(hg1e))
            hg(2,neigen)=CMPLX(hg2*DEXP(hg2e))
            hg(3,neigen)=CMPLX(hg3*DEXP(hg3e))
            if(iopt .eq. 2) then

...
c     12 Nov 1996   Changed to allow paths longer than 20000.
...
c     LWPC parameters
      ...
      parameter    (mxeigen=50,mxsgmnt=2001)
     ...
     &              hgt(3,mxeigen),hgr(3,mxeigen)
      ...
      dtr = REAL(pi) / 180.
      alpha = 2. / REAL(r_earth)
c      write(*,*) "LW_SUM CALLED"

      if (ijkl .eq. 1) then

         omega=1.e3*REAL(twopi)*freq
         waveno=1.e6*REAL(twopi/c_speed)*freq
         mik=CMPLX(0.,-waveno)
         aconst=-8686.*waveno
...
c        Antenna orientation factors
         theta=headng-bearng
         t(1)=COS(incl*dtr)
         t(2)=SIN(incl*dtr)*SIN(theta*dtr)
         t(3)=SIN(incl*dtr)*COS(theta*dtr)
...
c        Get sum of modes
         mikx=mik*(dst-xone)
         factor=const/SQRT(ABS(SIN(dst/REAL(r_earth))))
         ...
         do nc=1,nrcmp
            ...
            tbr= REAL(tb)
            tbi=AIMAG(tb)
            phs2=ATAN2(tbi,tbr)*180./REAL(pi)

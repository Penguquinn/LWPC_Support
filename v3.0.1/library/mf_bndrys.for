...
c     LWPC parameters
     ...
     &              lprnt,lgprnt,mprnt,mrprnt,msprnt,mzprnt,rprnt,xprnt
      ...
      nbr=INT((rtbndy-bndylt)/gmax+0.99)
      width=(rtbndy-bndylt)/nbr
      nureal=INT(width/tmesh+0.99)
      adjmsh=width/nureal
      unitr=adjmsh
      uniti=adjmsh/SQRT(3.)
      hmax=bxrtio*width
      nuimax=INT(hmax/uniti+0.01)
...
Change: END
      ...
      do kr=1,nbr
         ...
         if (autos .ne. 0) then
            argnum=capa
            argden=capk*COS(trht*dtr)
            if (argnum .lt. 4.9*argden) then
               sinhti=argnum/argden
               thetai=-LOG(sinhti+SQRT(sinhti**2+1.))*rtd
               if (thetai .lt. bndylw) thetai=bndylw
               deli=tpbndy-thetai
            end if
         end if

         nuiall=INT(deli/uniti+0.99)
         nbi=1
...
c        Check for Brewster mode
         ...
         tpr=INT( REAL(tp)*10.*rtd)/10.
         tpi=INT(AIMAG(tp)*10.*rtd)/10.-.1

...
c     LWPC parameters
      ...
      integer       rprnt,xprnt
      real          omega,wavenr,amp(2,2),phs(2,2),rtd,xlog

      common/mf_emtx/
     &              eu(2,2),ed(2,2),deu(2,2),ded(2,2),
     &              eiu(2,2),eid(2,2),deiu(2,2),deid(2,2)
     &      /mf_mode/
     &              theta,c,s,csq,ssq,omega,wavenr,ideriv
     &      /mf_oemx/
     &              r(2,2),dr(2,2),temp(2,2),dtemp(2,2),nrt
     &      /mf_prnt/
     &              lprnt,lgprnt,mprnt,mrprnt,msprnt,mzprnt,rprnt,xprnt
     &      /mf_rntg/
     &              x(2,2),dx(2,2),dxdh(8),roe(2,2),droe(2,2),
     &              rnum11,rnum21,rnum12,rnum22,rden
      ...
      if (mrprnt .gt. 0 .and. nrt .gt. 0) then
         ...
         cxlog=LOG(roe(1,1))
         amp(1,1)= REAL(cxlog)*xlog
         phs(1,1)=AIMAG(cxlog)*rtd
         cxlog=LOG(roe(2,1))
         amp(2,1)= REAL(cxlog)*xlog
         phs(2,1)=AIMAG(cxlog)*rtd
         cxlog=LOG(roe(1,2))
         amp(1,2)= REAL(cxlog)*xlog
         phs(1,2)=AIMAG(cxlog)*rtd
         cxlog=LOG(roe(2,2))
         amp(2,2)= REAL(cxlog)*xlog
         phs(2,2)=AIMAG(cxlog)*rtd

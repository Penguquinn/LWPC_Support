...
c  Change History:
c     13 Nov 1990   Dropped the equivalence structure in printout of
c                   MONTH, DAY, YEAR and UT.

c     14 Mar 1991   When printing, output YEAR modulo 100

c     29 Jan 1994   Added ASCII/BINARY option.

c     08 Feb 1994   Added error checking on write statements.

c     06 Apr 1994   Dropped printout of STNDEV; changed from list
c                   directed output to formatted output.

c     21 Oct 1995   Changed to get the LOG unit from LWPC_LUN.CMN.

c     30 Jan 1996   Added coding for extra arrays to be written;
...
c        Set the number of extra arrays to be written
         ...
         if (print_grd .gt. 0) then
            if (ncmp .eq. 0) then
               param_07=INT(param( 7))
               param_08=INT(param( 8))
               param_09=INT(param( 9))
               param_10=INT(param(10))

               write(lwpcLOG_lun,
     &             '(i2,f6.0,f6.0,f8.1,2f6.1,2x,
     &               2(i2.2,''/''),i2.2,'':'',i4.4,f7.0,f7.1,i4)')
     &               nrcmp,param(1),(param(i),i=3,6),
     &               param_07,param_08,MOD(param_09,100),param_10,

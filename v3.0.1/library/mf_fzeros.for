...
c     25 Nov 1996   Changed name of local UNITI to prevent changing the
...
c     also found by linear interpolation.
      ...
      fientr=
     & REAL(((thetab-thentr)*fia+(thentr-thetaa)*fib)/(thetab-thetaa))
...
c        THETAB, in turn, is always counterclockwise from THETAA.
     ...
     &                  ''No exit from mesh unit'')')
               ...
               fiexit=REAL(((thetaa-thexit)*fic+(thexit-thetac)*fia)
     &                /(thetaa-thetac))
            end if
         else
            ...
            fiexit=REAL(((thetac-thexit)*fib+(thexit-thetab)*fic)
     &             /(thetac-thetab))
         end if
...
c        round off errors in determining THETAA, THETAB and THETAC.
     ...
     &               ''Too many zeros in the rectangle'')')
         ...
         if (nzl .gt. 1) then
            if (lprnt .ne. 0 .or. mzprnt .ne. 0)
     &        write(lwpcLOG_lun,
     &            '(''MF_FZEROS: '',
     &              ''On same phase line:'',4(2x,2f7.3),
     &              '' - mesh size reduced'')')
     &             (thzero(i),i=1,nzl)
            go to 80
         end if

      SUBROUTINE NOISE_ITSN
     &          (rclt,rlng,freq,bandw,month,UT,
     &           Fam,sigm,Vd,sigv,Du,sigu,Dl,sigl,ampn,sign)

c***********************************************************************

c     Evaluates atmospheric noise values using the UT mapping coefficients
c     of Zacharisen and Jones, OT/ITS Report 2, 1970.
...
c  Change History:
c     27 May 1992   Added a new common block called LWPC_CFG, which is
c                   used to store configuration information, such as
c                   the location of the LWPC data files.

c     31 Jan 1994   Added error checking on open statement.

c     05 May 1995   Modified stdv to include Du and sigma Du

c     14 Oct 1995   Modified to output all of the available parameters
c                   including Vd and sigma Vd from Spaulding and
c                   Washburn, NTIA Report 85-173, 1985, p 151.

c     25 Oct 1995   Modified to use lwpc_LUN.cmn to set logical units.

c     05 Feb 1998   Modified to use OPEN_DAT.
...
c     LWPC parameters
      ...
      dimension     grid(1518),
     &              a(21,10,5),dud(5,12,5),fm(14,12),
     &              c(10),s(10),tl(21,5),time(5),b(8,2),
     &              vdc(5,6,4),svc(5,6,4)

      equivalence  (grid(1),a(1,1,1)),(grid(1051),dud(1,1,1)),
     &             (grid(1351),fm(1,1))
...
c           open the data file
            call OPEN_DAT (lwpcITSN_lun,file_name,'unformatted')
         end if
         do i=1,month
            read (lwpcITSN_lun) grid
         end do
         REWIND (lwpcITSN_lun)
         mosav=month
      end if
...
c     Latitude analysis
      ...
      do i=1,5
         ...
         do j=1,15
            ...
            if (MOD(j,2) .eq. 0) gkcs=gkcs*gcos
            tl(j,i)=(g+a(j,1,i))*gkcs
         end do
         mg=7
         km=5
         do j=16,21
...
c     Correct longitude to be East longitude
      rlon=rlng
      if (rlon .lt. 0.) then
         rlon=ABS(rlon)
      else
     &if (rlon .gt. 0.) then
         rlon=REAL(twopi)-rlon
      end if

c     0.0374 radians = 2.1428 degrees

      glon=rlon-.0374
      c(1)=COS(glon)
      s(1)=SIN(glon)
      do jj=2,10
         c(jj)=c(1)*c(jj-1)-s(1)*s(jj-1)
         s(jj)=c(1)*s(jj-1)+s(1)*c(jj-1)
      end do
      ...
      do i=1,5
         ...
         do jl=1,10
            jk=2*jl
            f=f+tl(jk,i)*s(jl)+tl(jk+1,i)*c(jl)
         end do
         time(i)=f
      end do
...
c     Fam at 1 MHz
      atno=time(1)+time(2)*s(1)+time(3)*c(1)+time(4)*(2.*s(1)*c(1))
     &    +time(5)*(c(1)*c(1)-s(1)*s(1))
...
c     12. / pi converts radians to hours at 15 degrees/hour

      rhr=hour+rlon*12./REAL(pi)
...
c     print '(''ITSN: rclt,rlng,month,hour,rhr,hr1,iblk1       '',
...
c        print '(''ITSN:                          hr ,iblk ,Fam,Fa,Vd'',
c    &           28x,f4.0,i3,3f8.3)',

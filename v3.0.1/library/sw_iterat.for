...
c  Change History:
c     21 Oct 1994   Added SW_PATH so we could use PRINT_SWG during
...
c     11 Sep 1995   Added test for very large F functions to stop
c                   when it's outrageous.

c     21 Oct 1995   Changed to get the LOG unit from LWPC_LUN.CMN.

c     18 Nov 1995   Added coding to find the value of BOTHT which
...
c     29 Oct 1996   Set increment for adjusting BOTHT to be consistent
c                   with LW_FULL_MC (WF_DRIVER).

c     06 Sep 1998   Modifed test for adjusting BOTHT to abort the
...
c     Store the magnitude of the F function for the starting angle
     ...
     &            theta,fmag,dlthta,dfdtht
         ...
         absr=REAL(ABS(REAL(dlthta)))
         absi=REAL(ABS(AIMAG(dlthta)))

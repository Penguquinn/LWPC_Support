...
c     05 Feb 1998   Added ACTION='READ' to the open statement.
...
c     Set local variables
      lu=logical_unit
      file_name=fname(:ISTR_LENGTH(fname))
      file_format=fform(:ISTR_LENGTH(fform))
...
c     Check unit status:
      ...
      if (exists) then
         ...
         open_name=root_name(:ISTR_LENGTH(root_name))//'.'//extension
...
c        may not match the value used in lwpcDAT_loc.
         if (open_name(:ISTR_LENGTH(open_name)) .eq.
     &       file_name(:ISTR_LENGTH(file_name))) then
...
c           File is already open but the name is incorrect
     ...
     &            ''Unit '',i2,'' is already opened as '',a,1x,
     &            ''instead of '',a)')
     &              lu,open_name(:ISTR_LENGTH(open_name)),
     &                 file_name(:ISTR_LENGTH(file_name))
            call LWPC_ERROR ('ERROR',error_msg)
         end if
      else
...
c        see if it is in lwpcDAT_loc
         full_name=lwpcDAT_loc(:ISTR_LENGTH(lwpcDAT_loc))//
     &             file_name(:ISTR_LENGTH(file_name))
...
c           see if it is in lwpcXTR_loc
            if (ISTR_LENGTH(lwpcXTR_loc) .gt. 0) then

               full_name=lwpcXTR_loc(:ISTR_LENGTH(lwpcXTR_loc))//
     &                   file_name(:ISTR_LENGTH(file_name))
               call STR_LOWER (full_name,0,0)
               INQUIRE (file=full_name,exist=exists)
            end if
...
c           The file is not found
     ...
     &          '(''[OPEN_DAT]: ''
     &            ''File not found: '',a)')
     &              full_name(:ISTR_LENGTH(full_name))
            call LWPC_ERROR ('ERROR',error_msg)
         end if
      end if
      RETURN
...
900   write(error_msg,
     ...
     &      ''file: '',a)')
     &      iocheck,full_name(:ISTR_LENGTH(full_name))
      call LWPC_ERROR ('Error',error_msg)

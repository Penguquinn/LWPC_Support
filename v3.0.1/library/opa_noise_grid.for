...
c  Change History:
c     03 Jun 1992   Removed references for the DECO noise model per
...
c     19 Jan 1994   Changed grd_dir to character*(*) and put in tests
c                   for the length of the grid directory.

c     21 Jan 1994   Dropped lwpcGRD_loc from argument list; does not
c                   open noise grid file; always calculates noise data.

c     29 Jan 1994   Added ASCII/BINARY option.

c     06 Apr 1994   Changed output to be dB; output the standard
c                   deviation as dB instead of weighting it by the
c                   signal amplitude.

c     30 Jun 1994   Dropped PRFL_ID, CXCLT and SXCLT from argument list.

c     30 Jan 1996   Added V sub d to the output grid file.
